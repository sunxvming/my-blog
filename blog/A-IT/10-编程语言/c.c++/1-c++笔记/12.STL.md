...
| 容器 | 头文件 | 底层结构 | 元素顺序 | 重复元素 | 尾部插入/删除 | 中间/头部插入删除 | 按值查找 | 随机访问 | 迭代器类型 | 内存连续性 | C++ 标准 |
|------|--------|----------|----------|----------|----------------|--------------------|----------|------------|--------------|--------------|-----------|
| `array` | `<array>` | 静态数组 | 用户决定 | 允许 | 不支持增删 | 不支持 | O(N) | O(1) | 随机访问 | 连续 | C++11 |
| `vector` | `<vector>` | 动态数组 | 用户决定 | 允许 | O(1) 均摊 | O(N) | O(N) | O(1) | 随机访问 | 连续 | C++98 |
| `deque` | `<deque>` | 分段连续数组 | 用户决定 | 允许 | O(1) | O(N) 但头/尾快 | O(N) | O(1) | 随机访问 | 逻辑连续 | C++98 |
| `list` | `<list>` | 双向链表 | 用户决定 | 允许 | O(1) | O(1) | O(N) | 不支持 | 双向 | 不连续 | C++98 |
| `forward_list` | `<forward_list>` | 单向链表 | 用户决定 | 允许 | O(1) | O(1) | O(N) | 不支持 | 前向 | 不连续 | C++11 |


## 二、关联容器（Associative Containers）  
（通常基于红黑树，**自动排序**）

| 容器 | 头文件 | 底层结构 | 元素顺序 | 重复键/值 | 插入/删除/查找平均 | 随机访问 | 迭代器类型 | 内存连续性 | C++ 标准 |
|------|--------|----------|----------|------------|--------------------|----------|--------------|--------------|-----------|
| `set` | `<set>` | 红黑树 | 自动升序 | 不允许重复元素 | O(log N) | 不支持 | 双向 | 不连续 | C++98 |
| `multiset` | `<set>` | 红黑树 | 自动升序 | 允许重复 | O(log N) | 不支持 | 双向 | 不连续 | C++98 |
| `map` | `<map>` | 红黑树 | 按 Key 升序 | Key 唯一 | O(log N) | 不支持 | 双向 | 不连续 | C++98 |
| `multimap` | `<map>` | 红黑树 | 按 Key 升序 | Key 可重复 | O(log N) | 不支持 | 双向 | 不连续 | C++98 |


## 三、无序关联容器（Unordered Associative Containers）  
（基于哈希表，**不排序**，C++11 引入）

| 容器 | 头文件 | 底层结构 | 元素顺序 | 重复键 | 插入/删除/查找平均 | 最坏情况 | 随机访问 | 迭代器类型 | 内存连续性 | C++ 标准 |
|------|--------|----------|----------|--------|--------------------|----------|----------|--------------|--------------|-----------|
| `unordered_set` | `<unordered_set>` | 哈希表 | 无序 | 不允许 | O(1) | O(N) | 不支持 | 前向 | 不连续 | C++11 |
| `unordered_multiset` | `<unordered_set>` | 哈希表 | 无序 | 允许 | O(1) | O(N) | 不支持 | 前向 | 不连续 | C++11 |
| `unordered_map` | `<unordered_map>` | 哈希表 | 无序 | Key 唯一 | O(1) | O(N) | 不支持 | 前向 | 不连续 | C++11 |
| `unordered_multimap` | `<unordered_map>` | 哈希表 | 无序 | Key 可重复 | O(1) | O(N) | 不支持 | 前向 | 不连续 | C++11 |


## 四、容器适配器（Container Adapters）
（底层依赖其他容器，不直接遍历）

| 适配器 | 头文件 | 底层默认容器 | 可选底层 | 支持操作 | 是否支持迭代器 | 典型特点 | C++ 标准 |
|--------|--------|--------------|----------|----------|----------------|------------|-----------|
| `stack` | `<stack>` | `deque` | `vector` / `list` | `push`, `pop`, `top` | 否 | LIFO | C++98 |
| `queue` | `<queue>` | `deque` | `list` | `push`, `pop`, `front`, `back` | 否 | FIFO | C++98 |
| `priority_queue` | `<queue>` | `vector` | `deque` | `push`, `pop`, `top` | 否 | 最大堆 | C++98 |


## 六、重要特性对比总结

| 特性 | `array` | `vector` | `deque` | `list` | `set` / `map` | `unordered_*` |
|------|----------|-----------|----------|---------|----------------|----------------|
| 大小是否固定 | ✅ 固定 | ❌ 动态 | ❌ 动态 | ❌ 动态 | ❌ 动态 | ❌ 动态 |
| 内存是否连续 | ✅ | ✅ | 分段连续 | ❌ | ❌ | ❌ |
| 中间插入高效性 | ❌ | ❌ | ❌ | ✅ | ✅（红黑树） | ✅（哈希） |
| 查找高效 | ❌ O(N) | ❌ O(N) | ❌ O(N) | ❌ O(N) | ✅ O(log N) | ✅ O(1) 均摊 |
| 是否支持排序后保持 | ❌ 用户排序 | ❌ 需手动 | ❌ 需手动 | ❌ 需手动 | ✅ 自动排序 | ❌ 无序 |
| 支持 `[]` 下标 | ✅ | ✅ | ✅ | ❌ | ✅（map） | ✅（unordered_map） |
| 迭代器失效风险 | 无 | 高（扩容/中间插入） | 中 | 低（除被删元素外） | 低 | 中（重哈希） |
...
### ✅ 优先使用场景
- **需要快速随机访问** → `vector` / `array` / `deque`
- **频繁在头部/尾部插入删除** → `deque`
- **频繁在任意位置插入删除** → `list` / `forward_list`
- **需要自动按顺序存储** → `set` / `map`
- **需要快速查找且不关心顺序** → `unordered_set` / `unordered_map`
- **仅做 LIFO / FIFO** → `stack` / `queue`
- **需要优先队列（按优先级取）** → `priority_queue`
...
### 有序关联容器 
**顺序**容器的特点是，元素的次序是由它插入的次序而决定的，访问元素也就按照最初插入的顺序。
而**有序**容器则不同，它的元素在插入容器后就被按照某种规则自动排序，所以是“有序”的。
C++ 的有序容器使用的是树结构，通常是**红黑树**——有着最好查找性能的二叉树。
* set/multiset
* map/multimap
...
指针是“访问连续内存的地址”，而迭代器是“统一访问各种容器元素的抽象接口”。指针可以看作最简单的一种迭代器，而迭代器的设计目标是让 STL 算法不必关心容器的底层实现。  使算法和容器解耦

算法操作容器，但实际上它看到的并不是容器，而是指向起始位置和结束位置的迭代器，算法只能通过**迭代器**去“**间接**”访问容器以及元素，算法的能力是由迭代器决定的。
...
支持按位访问、设置、翻转、统计 1 的个数等操作，长度必须在编译期确定
```c++
template <size_t N>
class bitset
{
