...
2. 在创建对象的同时以对象赋值    `Student stu2 = stu1;  //以拷贝的方式初始化`
3. 函数的形参为类类型
4. 函数返回值为类类型
...
```
**深拷贝**的例子比比皆是，标准模板库（STL）中的 `string、vector、stack、set、map` 等都必须使用深拷贝。

深拷贝的时机：
1. 存在指针成员，和动态分配的内存, 因为只有这样，才能将指针指向的内容再复制出一份来，让原有对象和新生对象相互独立，彼此之间不受影响。

### 重载=（赋值运算符）
在定义的同时进行赋值叫做**初始化**（Initialization），定义完成以后再赋值（不管在定义的时候有没有赋值）就叫做**赋值**（Assignment）。初始化只能有一次，赋值可以有多次。
...
* 1.需要析构函数的类也需要拷贝和赋值操作。若没有拷贝和赋值操作，那么析构函数free()的时候可能free两次(即同时释放两次资源)，造成错误
...
3. **赋值操作符（Copy Assignment Operator Rule）：** 如果类需要在拷贝赋值操作符中执行深拷贝，并且为了支持对象的赋值操作，则需要定义拷贝赋值操作符。
...
#### static_pointer_cast
C++ 标准库为智能指针（主要是 std::shared_ptr）提供的一组类型转换函数之一。
static_pointer_cast 本质上是对 shared_ptr 内部保存的裸指针执行 static_cast，同时保持引用计数不变。

|智能指针版本|对应裸指针版本|用途|
|---|---|---|
|`static_pointer_cast`|`static_cast`|编译期转换|
|`dynamic_pointer_cast`|`dynamic_cast`|运行时安全转换|
|`const_pointer_cast`|`const_cast`|增删 const|
|`reinterpret_pointer_cast`|`reinterpret_cast`|强制重新解释内存|
#### const_cast
