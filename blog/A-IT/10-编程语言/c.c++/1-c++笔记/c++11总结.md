...
 lambda 表达式和变量一样是“一等公民”，用起来也就更灵活自由，能对它做各种运算，生成新的函数。这就像是数学里的复合函数那样，把多个简单功能的小lambda 表达式组合，变成一个复杂的大 lambda 表达式。
...
在 lambda 表达式赋值的时需用auto关键字，要不然你不知道他是什么类型
...
## 为什么 nullptr 比 NULL 更好？
答案就是：**`nullptr` 有独立类型 `std::nullptr_t`，而 `NULL` 只是一个宏，本质上可能还是整数 0 或 `(void*)0`。** 因此在 C++ 的重载解析和模板推导中会产生语义歧义。nullptr 就消除了这种歧义。
...
**explicit**
explicit 用于修饰类的 单参数构造函数，表示禁止编译器进行**隐式类型转换**。用于避免不意料的类型转换，提高代码的明确性和可读性。
...
### 右值引用
它解决了一个核心问题：
- 如何识别“即将销毁的对象”，从而安全地偷走它的资源，而不是进行昂贵的拷贝。
- 右值引用其实就是：给一个即将销毁的对象起个名字。
...
在 C++11 以前，同一个对象可能有多种不同的初始化写法：

```cpp
X t1 = v;      // 拷贝初始化（copy initialization）
X t2(v);       // 直接初始化（direct initialization）
X t3 = {v};    // 聚合初始化（aggregate initialization）
               // 或某些情况下的列表初始化
X t4 = X(v);   // 先构造临时对象，再拷贝初始化
```
...
虽然结果看起来一样，但它们的语义并不完全相同：
- 是否允许 `explicit` 构造函数参与；
- 是否会产生临时对象；
- 是否调用拷贝构造函数；
- 是否存在窄化转换（narrowing conversion）；
    

这使得 C++ 的初始化规则变得复杂且难以记忆。

#### 2. C++11 引入统一初始化

为了统一各种初始化语法，C++11 引入了花括号初始化（Brace Initialization）：
...
{1,2,3}
    ↓
构造 std::initializer_list<int>
    ↓
调用 vector(initializer_list)
```

## 6. 统一初始化的一个重要优势：防止窄化转换
...
1. 使用 `{}` 提供统一的初始化语法；
2. 支持通过 `std::initializer_list` 初始化对象和容器；
3. 避免窄化转换，提高类型安全；
4. 减少不同初始化形式之间的语义差异。
...
# c++14
C++14 是 C++11 的“完善版”，没有颠覆性特性。C++11 的基础上进行了“小修小补”，主要目的是简化使用、填补缺陷和提升性能。

[[c++11总结#泛型的 lambda(14)]]




# c++17
C++17 是一次功能丰富的重要更新，进一步简化了语法、增强了泛型编程能力，并引入了大量实用的标准库组件。


## 结构化绑定
允许将数组、元组、结构体等解构为独立变量：
```cpp
...
## 嵌套命名空间定义
`namespace A::B::C { ... }` 替代 `namespace A { namespace B { ... }}`
...
// 示例1：正常除法
std::optional<int> res1 = div_int(10, 2);
if (res1.has_value()) {
	std::cout << "10/2 = " << res1.value() << std::endl;
} else {
	std::cout << "除法出错（除数为0）" << std::endl;
}
// 示例2：除零情况
auto res2 = div_int(5, 0);
if (res2) {   // 直接转换为 bool
	std::cout << "5/0 = " << *res2 << std::endl;
} else {
	std::cout << "5/0 是无效的" << std::endl;
}
```


## std::variant(17)
类型安全的联合体（类似 union，但可存储多种类型并自动管理生命周期）。
```cpp
...
`string_view`提供一个字符串的视图，即可以通过这个类以各种方法“观测”字符串，但**不允许修改**字符串。它内部只保存**一个指针**和**长度**，无论是拷贝，还是修改，都非常廉价
...
## std::filesystem
基于 boost.filesystem，提供文件系统操作（路径、目录迭代、文件状态等）：

```cpp
namespace fs = std::filesystem;
for (auto& p : fs::directory_iterator(".")) std::cout << p.path();
```



# c++20
C++20 是 C++ 发展史上又一个里程碑，引入了诸多革命性特性，使 C++ 语言更现代、更安全、更高效。
...
- 协程：
	- 用户态 
	- C++20协程本质是编译器提供的一套“函数暂停/恢复机制”。
	- 编译器把函数转换成状态机，通过Coroutine Frame保存执行上下文，实现低成本挂起与恢复。
- 线程：
	- 内核态
...
Response handle_request()
{
    ...
    auto user = query_db(); // 数据库IO

    auto info = call_api(); // HTTP IO

    return make_response(user, info);
}
```
...
这就是著名的：Callback Hell（回调地狱）

### 协程版本
```cpp
task<Response> handle_request()
{
    auto req = co_await recv_async();

    auto user = co_await query_db_async();

    auto info = co_await call_api_async();

    co_return make_response(user, info);
}
```

本质：暂停 (Suspend) →保存上下文→ 恢复 (Resume)→继续执行
...
为什么需要Concept?  给模板增加约束，解决模板可读性问题
...
```
如果编译：sort(123);        报错：200行模板错误。完全看不懂。
...
使用：
```cpp
template<Sortable T>void func(T t){}
```

如果：编译  func(123);

报：int不满足Sortable    错误清晰很多。
...
```
template<std::integral T>void f(T t){}
```

floating_point: 只能浮点
```
template<std::floating_point T>void f(T t){}
```

## 范围（range）

Ranges是 STL 的现代化升级。
很多人第一次看到 Range，会觉得它只是把：`std::sort(v.begin(), v.end()); ` 变成： `std::ranges::sort(v);`
实际上这只是最表面的功能。

Range 真正的价值是：
把 STL 从“基于迭代器的算法库”升级成了“基于数据流（Pipeline）的算法库”。


Range 是对 Iterator 的高级封装，View 是一种惰性计算的 Range Adapter，通过组合多个 View 构建数据处理流水线，在遍历时才执行计算，从而避免中间容器和不必要的数据拷贝。